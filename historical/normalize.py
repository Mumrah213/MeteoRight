...
def json_to_forecast_df(
    ...
    *,
    ...
    # Ensure issue_time is tz-aware (UTC) for proper timedelta subtraction
    if issue_time.tzinfo is None:
        issue_dt = pd.Timestamp(issue_time, tz="UTC")
    else:
...
def _previous_run_key(variable: str, lead_day: int, hourly: dict) -> str | None:
    """Find the response key for a previous-runs variable/lead pair."""
    candidates = [
        variable if lead_day == 0 else f"{variable}_previous_day{lead_day}",
        f"{variable}_previous_day_{lead_day}",
        f"{variable}_day{lead_day}",
    ]
...
def json_to_previous_runs_forecast_df(
    response: dict,
    model: str,
    variables: tuple[str, ...],
    ...
    *,
    ...
    """Convert a Previous Runs API response to wide forecast rows.

    Previous Runs returns values at fixed lead-day offsets for each target time,
    e.g. ``temperature_2m_previous_day3``. This converter maps those columns
    back into the common forecast schema:

    - ``forecast_target_time`` is the API timestamp
    - ``lead_hours`` is ``lead_day * 24``
    - ``forecast_issue_time`` is target time minus lead hours
    """
    hourly = response.get("hourly", {})
    if not hourly or "time" not in hourly:
        raise DataError("Response missing 'hourly.time' array")
    ...
    for lead_day in lead_days:
        lead_hours = int(lead_day * 24)
        frame = pd.DataFrame(
            {
                "forecast_issue_time": target_times - pd.Timedelta(hours=lead_hours),
                "forecast_target_time": target_times,
                "lead_hours": lead_hours,
                "model": model,
                "latitude": response.get("latitude"),
        ...
        frame = _add_location_provenance(
            frame,
            location_id=location_id,
            location_name=location_name,
            requested_latitude=requested_latitude,
        ...
        for variable in variables:
            key = _previous_run_key(variable, lead_day, hourly)
            if key is None:
                logger.warning(
                    "Previous-runs variable '%s' lead_day=%d not found", variable, lead_day
                )
                frame[variable] = None
            else:
                frame[variable] = hourly[key]

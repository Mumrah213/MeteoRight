...
def write_forecasts(
    ...
    if df.empty:
        logger.debug("No forecast data to write")
        return 0
    ...
    for (model, year, month), group in df.groupby(["model", "year", "month"]):
        ...
        if append:
            ...
            if existing is not None:
                # Deduplicate by forecast identity and location. Latitude and
                # longitude are required for multi-city datasets sharing the
                # same model/year/month partition.
                ...
                existing = existing[list(existing_cols.intersection(set(group.columns)))]
        ...
        # PyArrow 23.x raises on re-registering pandas type extensions;
        # retry once — the first attempt usually registers successfully.
        table = None
        for _ in range(2):
            try:
                table = pa.Table.from_pandas(group, preserve_index=False)
                break
            except (pa.ArrowInvalid, pa.lib.ArrowException) as exc:
                if "type extension" in str(exc).lower():
                    continue
                raise
        if table is None:
            table = pa.Table.from_pandas(group, preserve_index=False)
        ...
        written += len(group)
        logger.debug("Wrote %d forecast rows → %s", len(group), path)
...
def write_observations(
    ...
    if df.empty:
        logger.debug("No observation data to write")
        return 0
    ...
    for (year, month), group in df.groupby(["year", "month"]):
        ...
        written += len(group)
        logger.debug("Wrote %d observation rows → %s", len(group), path)
...
def write_download_log(
    ...
    logger.debug("Logged download: %s", entry)
